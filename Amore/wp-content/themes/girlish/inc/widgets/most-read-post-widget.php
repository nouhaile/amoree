...
if ( ! class_exists( 'girlish_Most_Read_Post' ) ) :

     
    class Girlish_Most_Read_Post extends WP_Widget {
        /**
         * Sets up the widgets name etc
         */
        public function __construct() {
            $tp_widget_most_read_post = array(
                'classname'   => 'widget_most_read_post',
                'description' => esc_html__( 'Retrive most read posts.', 'girlish' ),
            );
            parent::__construct( 'girlish_most_read_post', esc_html__( 'TP : Most Read Posts', 'girlish' ), $tp_widget_most_read_post );
        }
        ...
        public function form( $instance ) {
            $tp_title      = isset( $instance['title'] ) ? ( $instance['title'] ) : esc_html__( 'Most Read Posts', 'girlish' );
            $tp_number     = isset( $instance['number'] ) ? absint( $instance['number'] ) : 5;
            $tp_category   = isset( $instance['category'] ) ? absint( $instance['category'] ) : '';
           ?>
