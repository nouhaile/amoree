    ...
    $(window).scroll(function() {
     ...
     $(window).resize(function() {
        if ( $(window).width() < 1024 )
            $('.center-aligned .site-branding').insertBefore('.center-aligned #site-navigation');
        else
            $('.center-aligned .site-branding').insertAfter('.center-aligned #site-navigation');            
    });
...
$('.test').on('click', '.page-numbers :not(.disabled) a', function(e){
    e.preventDefault();
    var link = $(this).attr('href');
    $.post( link, function( data ) {
        var content = $(data).filter('.test');
        var pagination = $(data).filter('.page-numbers');
        $('.page-numbers').html(pagination.html());
        $('.test').animate(
            {opacity: 0},
            500, 
            function(){
