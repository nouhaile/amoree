/*jshint -W065 */
var blossommagazineproRepeaterRow = function( rowIndex, container, label ){
	'use strict';
...
wp.customize.controlConstructor['blossom-magazine-repeater'] = wp.customize.Control.extend({
	ready: function(){
		'use strict';
		...
		// The current value set in Control Class (set in Blossom_Magazine_Control_Repeater::to_json() function)
	...
	setImageInRepeaterField: function( attachment ){
		...
		$targetDiv.find( '.blossom-magazine-image-attachment' ).html( '<img src="' + attachment.url + '">' ).hide().slideDown( 'slow' );
	...
	setFileInRepeaterField: function( attachment ){
		...
		$targetDiv.find( '.blossom-magazine-file-attachment' ).html( '<span class="file"><span class="dashicons dashicons-media-default"></span> ' + attachment.filename + '</span>' ).hide().slideDown( 'slow' );
	...
	removeImage: function( event ){
		...
		$targetDiv.find( '.blossom-magazine-image-attachment' ).slideUp( 'fast', function() {
	...
	removeFile: function( event ){
		...
		$targetDiv.find( '.blossom-magazine-file-attachment' ).slideUp( 'fast', function() {
			jQuery( this ).show().html( jQuery( this ).data( 'placeholder' ) );
		});
		$targetDiv.find( '.hidden-field' ).val( '' );
		$uploadButton.text( $uploadButton.data( 'label' ) );
	...
	addRow: function( data ){
		...
		var control       = this,
		    template      = control.repeaterTemplate(), // The template for the new row (defined on Blossom_Magazine_Control_Repeater::render_content() ).
		...
		if ( template ) {
			...
			newRow = new blossommagazineproRepeaterRow(
				control.currentIndex,
    ...
    $(document).on( 'click', 'input[type="font"]', function(){
        ...
        if( ( ! $this.hasClass('ajax-running') ) && $this.siblings( '.font-awesome-list' ).length < 1 ){
            $.ajax({
				...
				url : ajaxurl,
				data: {
					action             : 'blossom_magazine_get_fontawesome_ajax',
					blossom_magazine_customize_nonce: blossom_magazine_customize.nonce
				},
				beforeSend: function(){
