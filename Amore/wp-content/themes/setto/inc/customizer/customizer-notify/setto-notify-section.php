<?php
class Setto_Customizer_Notify_Section extends WP_Customize_Section {
	
	public $type = 'setto-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'setto_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'setto_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		$setto_customizer_notify_show_recommended_actions = get_option( 'setto_customizer_notify_show' );
		foreach ( $setto_customizer_notify_recommended_actions as $key => $setto_lite_customizer_notify_recommended_action ) {
			if ( $setto_customizer_notify_show_recommended_actions[ $setto_lite_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $setto_lite_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$setto_lite_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $setto_lite_customizer_notify_recommended_action['plugin_slug'] ) ) {
				$active = $this->check_active( $setto_customizer_notify_recommended_action['plugin_slug'] );
				$setto_lite_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $setto_lite_customizer_notify_recommended_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$setto_lite_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$setto_lite_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$setto_lite_customizer_notify_recommended_action['button_class'] = 'install-now button';
						$setto_lite_customizer_notify_recommended_action['button_label'] = $install_button_label;
						break;
					case 'activate':
						$setto_lite_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
						$setto_lite_customizer_notify_recommended_action['button_label'] = $activate_button_label;
						break;
					case 'deactivate':
						$setto_lite_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
						$setto_lite_customizer_notify_recommended_action['button_label'] = $setto_deactivate_button_label;
						break;
				}
			}
			$formatted_array[] = $setto_lite_customizer_notify_recommended_action;
		}// End foreach().

		$customize_plugins = array();

		$setto_lite_customizer_notify_show_recommended_plugins = get_option( 'setto_customizer_notify_show_recommended_plugins' );

		foreach ( $setto_customizer_notify_recommended_plugins as $slug => $plugin_opt ) {

			if ( ! $plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $setto_lite_customizer_notify_show_recommended_plugins[ $slug ] ) && $setto_lite_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
			...
			switch ( $active['needs'] ) {
				...
				case 'deactivate':
					...
					$ti_customizer_notify_recommended_plugin['button_label'] = $setto_deactivate_button_label;
					break;
			}
		...
		$json['total_actions']       = count( $setto_customizer_notify_recommended_actions );
		...
		<# if( data.recommended_actions.length > 0 || data.recommended_plugins.length > 0 ){ #>
			<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
				...
				<h3 class="accordion-section-title">
					...
					<# if( data.recommended_actions.length > 0 ){ #>
						<span class="setto-customizer-plugin-notify-actions-count">
							<span class="current-index">{{ data.recommended_actions[0].index }}</span>
							{{ data.total_actions }}
						</span>
					<# } #>
				</h3>
				<div class="setto-theme-recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_actions.length > 0 ){ #>
						<# for (action in data.recommended_actions) { #>
							<div class="setto-recommeded-actions-container epsilon-required-actions" data-index="{{ data.recommended_actions[action].index }}">
								<# if( !data.recommended_actions[action].check ){ #>
									<div class="setto-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_actions[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no setto-customizer-notify-dismiss-recommended-action" id="{{ data.recommended_actions[action].id }}"></span>
					...
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="setto-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="setto-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_plugins[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no setto-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>
