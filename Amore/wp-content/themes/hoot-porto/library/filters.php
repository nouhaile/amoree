...
function hoot_archive_title_filter( $title ) {
	...
	elseif ( is_search() )
		...
		$title = sprintf( esc_html__( 'Search results for: %s', 'hoot-porto' ), get_search_query() );

	elseif ( function_exists( 'bbp_is_single_user' ) && bbp_is_single_user() )
		$title = __( 'User Profile ', 'hoot-porto' );
	...
	if ( ( ( $page = get_query_var( 'paged' ) ) || ( $page = get_query_var( 'page' ) ) ) && $page > 1 ) {
		...
		$suffix = ' <span class="loop-title-suffix loop-title-paged">' . sprintf( __( '(Page %s)', 'hoot-porto' ), $paged ) . '</span>';
...
function hoot_untitled_post( $title ) {
	...
	if ( ! $title && ! is_singular() && in_the_loop() && ! is_admin() )
		$title = esc_html__( '(Untitled)', 'hoot-porto' );
