...
function hoot_get_wp_link() {
	return sprintf( '<a class="wp-link" href="%s">%s</a>', esc_url( __( 'https://wordpress.org', 'hoot-porto' ) ), esc_html__( 'WordPress', 'hoot-porto' ) );
}
endif;
...
function hoot_get_wp_theme_link( $link = '', $anchor = '' ) {
	...
	$title  = sprintf( __( '%s WordPress Theme', 'hoot-porto' ), hoot_data()->template_name );
...
function hoot_display_meta_info( $args = '', $context = '', $editlink = true ) {
	...
	if ( !empty( $display['author'] ) ) :
		$blocks['author']['label'] = __( 'By:', 'hoot-porto' );
		ob_start();
	...
	if ( !empty( $display['date'] ) ) :
		$blocks['date']['label'] = __( 'On:', 'hoot-porto' );
	...
	if ( !empty( $display['cats'] ) ) :
		...
		if ( !empty( $category_list ) ) :
			$blocks['cats']['label'] = __( 'In:', 'hoot-porto' );
			$blocks['cats']['content'] = $category_list;
		endif;
	endif;

	if ( !empty( $display['tags'] ) && get_the_tags() ) :
		$blocks['tags']['label'] = __( 'Tagged:', 'hoot-porto' );
		$blocks['tags']['content'] = ( ! get_the_tags() ) ? __( 'No Tags', 'hoot-porto' ) : get_the_tag_list( '', ', ', '' );
	endif;

	if ( !empty( $display['comments'] ) && comments_open() ) :
		$blocks['comments']['label'] = __( 'With:', 'hoot-porto' );
		ob_start();
		comments_popup_link(__( '0 Comments', 'hoot-porto' ),
							__( '1 Comment', 'hoot-porto' ),
							__( '% Comments', 'hoot-porto' ), 'comments-link', '' );
		$blocks['comments']['content'] = ob_get_clean();
	endif;

	if ( $editlink && $edit_link = get_edit_post_link() ) :
		$blocks['editlink']['label'] = '';
		$blocks['editlink']['content'] = '<a href="' . $edit_link . '">' . __( 'Edit This', 'hoot-porto' ) . '</a>';
	endif;
...
function hoot_developer_data( $commented = true ) {
	ob_start();
	echo intval( get_num_queries() ) . ' ' . esc_attr__( 'queries.', 'hoot-porto' ) . ' ';
	timer_stop(1);
	echo esc_html( __( ' seconds. ', 'hoot-porto' ) . ( memory_get_peak_usage(1) / 1024 ) / 1024 . 'MB' );
	$out = ob_get_contents();
	ob_end_clean();
	return $out;
}
