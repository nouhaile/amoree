...
class Hoot_Taxonomy_Field {
	...
	public function taxonomy_columns_header( $defaults ){
		$defaults['hoot_term_img']  = __( 'Featured Image', 'hoot-porto' );
		return $defaults;
	}
	...
	public function taxonomy_column( $columns, $column, $id ){
		if ( 'hoot_term_img' === $column ) {
			...
			$columns .= '<img data-id="'.$thumbnail_id.'" src="' . esc_url( $image ) . '" alt="' . esc_attr__( 'Thumbnail', 'hoot-porto' ) . '" class="hoot_term_img" width="35px" />';
		}
		return $columns;
	}
	...
	public function add_taxonomy_field( $term ) {
		...
		?><div class="form-field">
			<label for="hoot_term_img"><?php esc_html_e( 'Featured Image', 'hoot-porto' ); ?></label>
			...
			<button type="button" class="hoot_term_img_add button"><?php esc_html_e( 'Upload/Add image', 'hoot-porto' ); ?></button>
			<button type="button" class="hoot_term_img_remove button"><?php esc_html_e( 'Remove image', 'hoot-porto' ); ?></button>
		</div><?php
	}
