...
$hoot_porto_archive_postcounter = hoot_data( 'archive_postcounter' );
if ( empty( $hoot_porto_archive_postcounter ) )
	$hoot_porto_archive_postcounter = 0;
$hoot_porto_archive_postcounter++;
hoot_set_data( 'archive_postcounter', $hoot_porto_archive_postcounter );

if ( $hoot_porto_archive_postcounter == 1 ) :
?>
...
<article <?php hoot_attr( 'post', '', 'archive-mixed archive-mixed-block2 mixedunit-big' ); ?>>
	...
	<div class="entry-grid hgrid">
		...
		<?php if ( is_sticky() ) : ?>
			<div class="entry-sticky-tag invert-typo"><?php esc_html_e( 'Sticky', 'hoot-porto' ) ?></div>
		<?php endif; ?>

		<?php $img_size = apply_filters( 'hoot_porto_archive_imgsize', '', 'mixedbig' );
...
<article <?php hoot_attr( 'post', '', 'archive-mixed archive-mixed-block2 mixedunit-block2 hcolumn-1-2' ); ?>>
	...
	<div class="entry-grid hgrid">
		...
		<?php $img_size = apply_filters( 'hoot_porto_archive_imgsize', 'hoot-large-thumb', 'mixedblock2' );
		hoot_post_thumbnail( 'entry-content-featured-img entry-grid-featured-img', $img_size, true, esc_url( get_permalink() ) ); ?>
